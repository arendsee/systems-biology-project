...
\begin{description}
  \item[ERP004034] inflorecence meristem, floral meristem and flower 
  \item[ERP005391] three developmental stages of somatic embryo development
  \item[SRP003864] copper defficiency (WT and spl7-KO) X (normal, low [Cu])

      Deleted spl7 mutants

  \item[SRP007113] female gametophyte
  \item[SRP009369] root hair 
  \item[SRP010642] brassinosteroid and gibberillin treatments, 
  \begin{description}
    \item[PAC-PPZ] negative for both the GA and BR hormones
    \item[PAC-GA]  negative for BR
    \item[PAC]     negative for GA
  \end{description}
      Deleted both bzr1 mutants and the PAC-GA runs (the controls)

  \item[SRP018404] root response to nitrate

      Deleted the sRNA runs

  \item[SRP027256] drought condition versus WT

      Deleted the GI mutants

  \item[SRP034715] Response to mildew (huge)

      Deleted the water control and the avirulent Emoy2 infected runs.

  \item[SRP035269] seed development (3 lines) X (3 dev stages) X (3 bioreps)

      Deleted lines CL37 and PDAT\_7, keeping only Fae 

  \item[SRP044814] Fe stress
  \item[SRP063017] Nematode infection

      Deleted cytokine hypersensitive type-A lines, 
\end{description}

SRP051665 is interesting in that it attempted to study the translatome by
extracting mRNAs from polyribosomes.

Honorable mentions, SRP032274 also uses PPZ to stop BR hormongering. SRP032366
heat stress, 38C for 15 minutes, deleted for lack of biological replicates.
