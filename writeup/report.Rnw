...
\begin{figure}[!ht] \centering
<<echo=F, fig.width=7, fig.height=7, cache=TRUE>>=
plot(g, vertex.label=NA)
@
  \caption{Orphan graph, Aracne cutoff of 0.5.}
  \label{fig:precut}
\end{figure}
...
@
  \caption{Orphan graph, Aracne cutoff of 0.5, cutting all connections between non-TF.}
  \label{fig:postcut}
\end{figure}
...
<<echo=FALSE>>=
# em <- read.table('../OUTPUT/matrices/expression.mat', header=T, row.names=1)
...
\subsection{Non-orphan control results}

<<>>=
tfs <- read.delim('../OUTPUT/tf.tab')

g <- read.graph('../OUTPUT/tf-orf-output/c0.5/r1.mat', format='ncol')
...
plot(g, vertex.label=NA)
@
  \caption{Non-orphan graph, Aracne cutoff of 0.5.}
  \label{fig:precut_non}
\end{figure}
...
plot(g2, vertex.label=NA)
@
  \caption{Non-orphan graph, Aracne cutoff of 0.5, cutting all connections between non-TF.}
  \label{fig:postcut_non}
\end{figure}
...
%
% \begin{figure}[!ht] \centering
% <<echo=FALSE, fig.width=4, fig.height=4, message=FALSE>>=
% require(ggplot2)
